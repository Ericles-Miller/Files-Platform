...
export class UpdateFolderUseCase {
  ...
  async execute({
    displayName, id, userId, parentId,
  }: IUpdateFolderDTO): Promise<void> {
    try {
      ...
      if (parentId) {
        ...
        const getOldParentFolder: Folders = await this.foldersRepository.findById(id);

        await this.foldersRepository.update(id, folder);

        if (getOldParentFolder.parentId) {
          this.calcSizeFoldersUseCase.execute(getOldParentFolder.parentId);
        }
